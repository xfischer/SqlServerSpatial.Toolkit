...
namespace SqlServerSpatial.Toolkit.BaseLayer
{
	public static class BingMapsTileSystem
	{
		...
		public static void LatLongToDoubleXY(double latitude, double longitude, out double pixelX, out double pixelY)
		{
			latitude = Clip(latitude, MinLatitude, MaxLatitude);
			longitude = Clip(longitude, MinLongitude, MaxLongitude);
			...
			double x = (longitude + 180) / 360;
			double sinLatitude = Math.Sin(latitude * Math.PI / 180);
			double y = Math.Log((1 + sinLatitude) / (1 - sinLatitude)) / (4 * Math.PI);
